...
function loadStatsPage(){
	...
	for(row = 2; row < table.rows.length; row++){
		//window.alert("Row: " + row + " : columns: " + table.rows[row].cells.length);
		//window.alert(table.rows[row].cells.length);
		var scoreA = parseInt(table.rows[row].cells[2].innerHTML);
		var scoreB = parseInt(table.rows[row].cells[3].innerHTML);
		//window.alert(scoreA + ", " + scoreB);
		if(scoreA > scoreB){
			//window.alert("Won");
			table.rows[row].cells[4].innerHTML = "CU Buffs";
			wins++;
		}else{
			//window.alert("Lost");
...
function loadPlayersPage(){
	...
	for (var i = 0; i < 4; i++) {
		...
		a.href = "#" + players[i].name;
		var link = document.createTextNode(players[i].name);
		a.appendChild(link);
		a.style.display = "block";
		a.style.color = "black";
		a.onclick = (function(playerNum){
        return function (){
            switchPlayers(playerNum);
        }
    })(i);
		select.appendChild(a);
		//window.alert("Created element");
	}
}

function switchPlayers(playerNum){
	document.getElementById("p_year").innerHTML = players[playerNum].major;
